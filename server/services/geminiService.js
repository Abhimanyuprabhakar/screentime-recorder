...
class GeminiCategorizationService {
  constructor() {
    this.provider = (process.env.AI_PROVIDER || "fallback").toLowerCase();
    this.groq =
  ...
  isConfigured() {
    if (this.provider === "groq") return Boolean(this.groq);
    if (this.provider === "openai") return Boolean(process.env.OPENAI_API_KEY);
    return false;
  }

  async requestClassification(prompt) {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert web activity classifier. Respond only with valid JSON."
          },
  ...
  async categorizeActivity(url, title, domain) {
    try {
      ...
      if (!this.isConfigured() || this.shouldUseFallback()) {
      ...
      let safeUrlContext = "Not provided";
      if (url) {
        try {
          const parsedUrl = new URL(url);
          safeUrlContext = `${parsedUrl.origin}${parsedUrl.pathname}`;
        } catch {
          safeUrlContext = domain;
        }
      }
...
**Domain:** ${domain}
**URL context without query parameters:** ${safeUrlContext}
...
- **tags**: 3-5 lowercase, descriptive tags for the content`;
      ...
      const text = await this.retryWithBackoff(() =>
        this.requestClassification(prompt)
      );
