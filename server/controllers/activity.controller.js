...
export const logActivity = async (req, res) => {
  try {
    ...
    switch (action) {
      ...
      case "update":
        await updateActivitySession(req.user.id, sessionId, duration);
        break;

      case "end":
        await endActivitySession(req.user.id, sessionId, endTime, duration);
        break;
...
async function updateActivitySession(userId, sessionId, duration) {
...
async function endActivitySession(userId, sessionId, endTime, finalDuration) {
  const result = await Activity.findOneAndUpdate(
    { userId, sessionId, isActive: true },
    {
...
export const getActivitySummary = async (req, res) => {
  try {
    ...
    const { startDate, endDate, groupBy = "domain", timeRange } = req.query;
    ...
    if (startDate || endDate) {
      ...
      matchQuery.startTime = { $gte: rangeStart };
    }
